...
function AppointmentListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AppointmentInterface[]>(
    ...
    () =>
      getAppointments({
        relations: ['user_appointment_patient_coordinator_idTouser', 'user_appointment_medical_staff_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_patient_coordinator_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_appointment_medical_staff_idTouser</Th>
                  )}
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link
                          as={NextLink}
                          href={`/users/view/${record.user_appointment_patient_coordinator_idTouser?.id}`}
                        >
                          {record.user_appointment_patient_coordinator_idTouser?.email}
                        </Link>
                      </Td>
                    )}
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_appointment_medical_staff_idTouser?.id}`}>
                          {record.user_appointment_medical_staff_idTouser?.email}
                        </Link>
                      </Td>
                    )}
